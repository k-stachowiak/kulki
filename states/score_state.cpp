...
void ScoreState::reset(const std::vector<std::pair<int, int>>& changes, bool next_deal)
{
    ...
    for (const auto& change : changes) {
        success |= m_context->m_board.find_streak(
                ...
                m_context->m_constants.score_serie_min);
    }
    ...
    ++m_context->m_streak;
    m_time = m_context->m_constants.score_period;

    int x_sum = 0, y_sum = 0;
    for (const auto& p : scored) {
        m_context->m_board(p.first, p.second) = m_context->m_constants.empty_field;
        x_sum += p.first;
        y_sum += p.second;
    }
...
void ScoreState::draw(double)
{
    ...
    al_draw_textf(
        m_context->m_constants.score_font, m_context->m_constants.score_color,
